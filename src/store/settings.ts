...
if (typeof window !== 'undefined' && !isMiddleScreen.value && isSidebarCollapsedConfig.value) {
  // 这是 QoQClashD 版的桌面导航默认行为:概览、节点、规则、日志等入口必须直接可见,
  // 不应该被旧版 zashboard 的默认收起状态继续遮住。只在桌面端强制展开，移动端仍保留折叠。
  isSidebarCollapsedConfig.value = false
}
