...
export default defineComponent({
  ...
  setup() {
    ...
    const hasTabs = computed(() => {
      return ruleProviderList.value.length > 0 || rules.value.length > 0
    })
    ...
    const tabsWithNumbers = computed(() => {
      const tabs: Array<{ type: RULE_TAB_TYPE; count: number }> = []
      if (rules.value.length > 0 || ruleProviderList.value.length > 0) {
        tabs.push({ type: RULE_TAB_TYPE.RULES, count: rules.value.length })
        if (ruleProviderList.value.length > 0) {
          tabs.push({ type: RULE_TAB_TYPE.PROVIDER, count: ruleProviderList.value.length })
        }
      }
      tabs.push({ type: RULE_TAB_TYPE.INBOUND, count: 0 })
      return tabs
    })
