<template>
  <div class="flex h-full flex-col overflow-hidden">
    ...
    <template v-if="!editingId">
      ...
      <footer class="border-base-300/50 space-y-2 border-t p-2">
        <button
          class="btn btn-sm btn-ghost w-full justify-center"
          :disabled="!proxyGroupList.length"
          @click="onSuggestRegions"
        >
          {{ $t('folder_suggest_regions') }}
        </button>
...
const onSuggestRegions = () => {
  const suggestions = buildRegionSuggestionFolders(proxyGroupList.value)

  if (!suggestions.length) return

  for (const suggestion of suggestions) {
    const existing = folders.value.find((folder) => folder.name === suggestion.name)

    if (!existing) {
      const created = createFolder(suggestion.name)
      created.manualIncludes = [...suggestion.manualIncludes]
      created.rules = [...suggestion.rules]
      continue
    }

    existing.manualIncludes = Array.from(
      new Set([...existing.manualIncludes, ...suggestion.manualIncludes]),
    )
  }
}
