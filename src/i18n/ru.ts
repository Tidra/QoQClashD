...
const ru: Partial<LANG_MESSAGE> = {
  ...
  proxyGroupEditorSelectedLive: 'Переключено на «{name}», ядро применило сразу',
  proxyGroupEditorSelectedDraft:
    '«{name}» сохранён как узел по умолчанию — после применения конфигурации',
  ...
  assetLogs: 'Логи загрузки ресурсов',
  ...
  IPInfoAPITip:
    'Этот API используется для определения геолокации IP-адресов в деталях соединения и DNS-запросах панели. Проверка IP использует собственные настройки API.',
  ...
  ruleProviderUpdate: 'Скачать обновление',
  ruleProviderUpdated: 'Набор правил «{name}» обновлён',
