<template>
  <div class="relative size-full overflow-hidden" :class="[disableProxiesPageTextSelect ? 'select-none' : '']">
    <!-- 节点列：节点 子页保持挂载，非 nodes 时隐藏；z-0 位于 folders/groups/subscriptions overlay 之下 -->
    <div
    ...
    <!-- 文件夹列：非 nodes 时保持挂载，隐藏；z-10 位于 nodes 列之上 -->
    <div
      v-show="nodeNavigation !== 'nodes'"
      class="absolute inset-0 z-10 h-full overflow-hidden"
    >
      <FolderManagerPanel v-if="foldersUiVisible && folderManagerOpen" />
      <div
        class="max-md:scrollbar-hidden relative h-full min-w-0 flex-1 flex flex-col"
    ...
    <!-- 代理组：保持挂载，切换仅显隐，避免重建闪烁；z-20 位于 folders/nodes 列之上 -->
    <div
    ...
    <!-- 订阅：保持挂载，切换仅显隐，避免重建闪烁；z-20 位于 folders/nodes 列之上 -->
    <div
