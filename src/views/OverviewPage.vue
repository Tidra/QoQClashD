<template>
  ...
  >
    <div class="px-3 pt-3">
      <div class="card bg-base-100/80 border-base-300/70 rounded-2xl border p-4 shadow-sm backdrop-blur-sm">
        <div class="flex items-center justify-between gap-3">
          <div>
            <div class="text-[10px] font-semibold uppercase tracking-[0.2em] text-base-content/60">
              QoQClashD
            </div>
        ...
        <div class="mt-4 grid gap-2 sm:grid-cols-3">
          <div class="rounded-xl border border-base-300/70 bg-base-200/40 p-3">
            <div class="text-[10px] uppercase tracking-[0.18em] text-base-content/50">后端</div>
            <div class="mt-2 text-sm font-medium text-base-content">{{ backendLabel }}</div>
            <div class="mt-1 text-xs text-base-content/55">{{ backendUrl }}</div>
          </div>
          <div class="rounded-xl border border-base-300/70 bg-base-200/40 p-3">
            <div class="text-[10px] uppercase tracking-[0.18em] text-base-content/50">内核</div>
            <div class="mt-2 flex items-center gap-2 text-sm font-medium text-base-content">
              <img
                :src="coreBrand.logo"
            ...
            <div class="mt-1 text-xs text-base-content/55">{{ coreVersionLabel }}</div>
          </div>
          <div class="rounded-xl border border-base-300/70 bg-base-200/40 p-3">
            <div class="text-[10px] uppercase tracking-[0.18em] text-base-content/50">状态</div>
            <div class="mt-2 text-sm font-medium text-base-content">
      ...
      <div class="card border-warning/40 bg-warning/10 rounded-2xl border p-4 shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div class="text-base-content/70 text-sm">核心将直接下载到配置的核心目录，下载完成后可在设置中启动。</div>
          </div>
          <button
            ...
            @click="downloadCore"
          >
            <span v-if="downloadingCore" class="loading loading-spinner loading-xs"></span>
