...
import { useControlInfo } from '~/composables/useControlInfo'
...
const { info: controlInfo } = useControlInfo()
...
const runtimeDirectories = computed(() => controlInfo.value?.runtime ?? null)
const runtimeRootInput = ref('')
watch(
  () => runtimeDirectories.value?.root,
  (value) => {
    if (value) runtimeRootInput.value = value
  },
  { immediate: true },
)

async function saveRuntimeRoot() {
  const root = runtimeRootInput.value.trim()
  if (!root) return
  await controlApi.setRuntimeRoot(root)
  await controlApi.ensureKernel()
  window.location.reload()
}
...
<template>
  <div class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-1">
    ...
    <template v-else>
      ...
      <div
        v-if="runtimeDirectories"
        class="card border-base-content/10 bg-base-100/80 shadow-sm"
      >
        <div class="flex items-center gap-2 border-b border-base-content/5 bg-base-300/30 px-4 py-3 text-sm font-semibold">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="size-5"
            viewBox="0 0 24 24"
        ...
        <div class="grid gap-3 p-4 text-xs">
          <div class="rounded-lg bg-base-200/60 p-3">
            <div class="mb-2 flex items-center justify-between gap-2">
              <div class="font-medium opacity-70">程序根目录</div>
              <button class="btn btn-xs btn-primary" @click="saveRuntimeRoot">
                保存根目录并启动核心
              </button>
            </div>
            <input
              v-model="runtimeRootInput"
              class="input input-sm input-bordered w-full bg-base-100 text-[11px]"
              placeholder="例如：D:/QoQClashD/runtime"
            />
          </div>
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="rounded-lg bg-base-200/60 p-3">
              <div class="mb-1 font-medium opacity-70">核心目录</div>
              <div class="break-all text-[11px]">{{ runtimeDirectories.kernel }}</div>
            </div>
            <div class="rounded-lg bg-base-200/60 p-3">
              <div class="mb-1 font-medium opacity-70">配置目录</div>
              <div class="break-all text-[11px]">{{ runtimeDirectories.config }}</div>
            </div>
            <div class="rounded-lg bg-base-200/60 p-3">
              <div class="mb-1 font-medium opacity-70">Profiles 目录</div>
              <div class="break-all text-[11px]">{{ runtimeDirectories.profiles }}</div>
            </div>
            <div class="rounded-lg bg-base-200/60 p-3">
              <div class="mb-1 font-medium opacity-70">Active Config</div>
              <div class="break-all text-[11px]">{{ runtimeDirectories.activeConfig }}</div>
            </div>
          </div>
        </div>
      </div>
