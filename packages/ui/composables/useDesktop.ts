...
export interface DesktopKernelBridge {
  defaultDir?: string
  downloadAndStart?: (options: {
    url: string
    fileName: string
    targetDir?: string
    platform?: string
    arch?: string
  }) => Promise<{ path?: string; started?: boolean }>
  start?: (binaryPath: string) => Promise<{ ok?: boolean }>
}
